...
void Init(QWidget* root) {
    ...
    // build version
    const auto branch_name = std::string(static_cast<const char*>(Common::g_scm_branch));
    const auto description = std::string(static_cast<const char*>(Common::g_scm_desc));
    const auto build_id = std::string(static_cast<const char*>(Common::g_build_id));

    const auto yuzu_build = std::format("Eden Development Build | {}-{}", branch_name, description);
