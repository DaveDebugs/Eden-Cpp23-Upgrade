...
std::optional<Common::Net::Release> UpdateChecker::GetUpdate() {
    ...
    boost::split(result, std::string{static_cast<const char*>(Common::g_build_version)}, boost::is_any_of("-"));
    if (result.empty())
        return std::nullopt;
    ...
    const std::string build = static_cast<const char*>(Common::g_build_version);
#endif
