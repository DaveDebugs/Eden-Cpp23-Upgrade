...
Buffer::Buffer(BufferCacheRuntime& runtime, DAddr cpu_addr_, u64 size_bytes_)
    ...
    if (runtime.device.HasDebuggingToolAttached()) {
        buffer.SetObjectNameEXT(std::format("Buffer 0x{:x}", CpuAddr()).c_str());
    }
}
...
VkBufferView Buffer::View(u32 offset, u32 size, VideoCore::Surface::PixelFormat format) {
    ...
    auto it = std::ranges::lower_bound(views, std::make_tuple(offset, size, format), std::less<>{},
        [](const BufferView& view) {
            return std::make_tuple(view.offset, view.size, view.format);
        });
    if (it != views.end() && it->offset == offset && it->size == size && it->format == format) {
        return *it->handle;
    }
    it = views.insert(it, {
        .offset = offset,
...
private:
    ...
    void MakeAndUpdateIndices(u8* staging_data, size_t quad_size, u32 quad, u32 first) override {
        constexpr std::array<u8, 6> idx{0, 1, 2, 0, 2, 3};
        switch (index_type) {
        case VK_INDEX_TYPE_UINT8_EXT: {
            auto* dst = reinterpret_cast<u8*>(staging_data);
            for (auto i : std::views::iota(0, 6)) dst[i] = static_cast<u8>(first + idx[i] + quad * 4);
            break;
        }
        case VK_INDEX_TYPE_UINT16: {
            auto* dst = reinterpret_cast<u16*>(staging_data);
            for (auto i : std::views::iota(0, 6)) dst[i] = static_cast<u16>(first + idx[i] + quad * 4);
            break;
        }
        case VK_INDEX_TYPE_UINT32: {
            auto* dst = reinterpret_cast<u32*>(staging_data);
            for (auto i : std::views::iota(0, 6)) dst[i] = static_cast<u32>(first + idx[i] + quad * 4);
            break;
        }
        default:
            ASSERT(false);
...
private:
    ...
    void MakeAndUpdateIndices(u8* staging_data, size_t quad_size, u32 quad, u32 first) override {
        constexpr std::array<u8, 6> idx{0, 3, 1, 0, 2, 3};
        switch (index_type) {
        case VK_INDEX_TYPE_UINT8_EXT: {
            auto* dst = reinterpret_cast<u8*>(staging_data);
            for (auto i : std::views::iota(0, 6)) dst[i] = static_cast<u8>(first + idx[i] + quad * 2);
            break;
        }
        case VK_INDEX_TYPE_UINT16: {
            auto* dst = reinterpret_cast<u16*>(staging_data);
            for (auto i : std::views::iota(0, 6)) dst[i] = static_cast<u16>(first + idx[i] + quad * 2);
            break;
        }
        case VK_INDEX_TYPE_UINT32: {
            auto* dst = reinterpret_cast<u32*>(staging_data);
            for (auto i : std::views::iota(0, 6)) dst[i] = static_cast<u32>(first + idx[i] + quad * 2);
            break;
        }
        default:
            ASSERT(false);
