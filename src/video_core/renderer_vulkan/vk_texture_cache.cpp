...
[[nodiscard]] boost::container::small_vector<VkBufferImageCopy, 16> TransformBufferImageCopies(
    ...
    auto maker = [buffer_offset](VkImageAspectFlags aspect) {
        return [buffer_offset, aspect](const BufferImageCopy& copy) {
            return VkBufferImageCopy{
    ...
    if (aspect_mask == (VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT)) {
        auto depth = copies | std::views::transform(maker(VK_IMAGE_ASPECT_DEPTH_BIT));
        auto stencil = copies | std::views::transform(maker(VK_IMAGE_ASPECT_STENCIL_BIT));
        boost::container::small_vector<VkBufferImageCopy, 16> result;
        result.reserve(copies.size() * 2);
        std::ranges::copy(depth, std::back_inserter(result));
