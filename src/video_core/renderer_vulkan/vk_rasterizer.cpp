    ...
    PrepareDraw(is_indexed, [is_indexed, instance_count](this auto& self) {
        ...
        if (GPU::Logging::IsActive() &&
            ...
            const std::string params = is_indexed ?
                std::format("vertices={}, instances={}, firstIndex={}, baseVertex={}, baseInstance={}",
                ...
                std::format("vertices={}, instances={}, firstVertex={}, firstInstance={}",
                    draw_params.num_vertices, draw_params.num_instances,
                    draw_params.base_vertex, draw_params.base_instance);
...
void RasterizerVulkan::DrawIndirect() {
    ...
    PrepareDraw(params.is_indexed, [&params](this auto& self) {
        ...
        if (GPU::Logging::IsActive() &&
            Settings::values.gpu_log_vulkan_calls.GetValue()) {
            const std::string log_params = std::format("drawCount={}, stride={}",
...
void RasterizerVulkan::DrawTexture() {
    ...
    const auto ScaleSrc = [&](auto dim_f) -> s32 {
        if (!src_rescaling) return static_cast<s32>(dim_f);
        return static_cast<s32>(dim_f * Settings::values.resolution_info.up_scale) >> Settings::values.resolution_info.down_shift;
    };

    const auto ScaleDst = [&](auto dim_f) -> s32 {
        if (!dst_rescaling) return static_cast<s32>(dim_f);
        return static_cast<s32>(dim_f * Settings::values.resolution_info.up_scale) >> Settings::values.resolution_info.down_shift;
    };
...
void RasterizerVulkan::DispatchCompute() {
    ...
    if (GPU::Logging::IsActive() &&
        Settings::values.gpu_log_vulkan_calls.GetValue()) {
        const std::string params = std::format("groupCountX={}, groupCountY={}, groupCountZ={}",
            dim[0], dim[1], dim[2]);
        GPU::Logging::GPULogger::GetInstance().LogVulkanCall(
