...
template <typename T>
struct SafeUnderlyingType<T, true> {
    using type = std::underlying_type_t<T>;
};

template <std::size_t Position, std::size_t Bits, typename T, typename EndianTag, typename CharT>
struct std::formatter<BitField<Position, Bits, T, EndianTag>, CharT> {
    using FormatType = typename SafeUnderlyingType<T>::type;
    ::std::formatter<FormatType, CharT> base_formatter;

    constexpr auto parse(auto& ctx) {
        return base_formatter.parse(ctx);
    }

    auto format(const BitField<Position, Bits, T, EndianTag>& bitfield, auto& ctx) const {
        return base_formatter.format(static_cast<FormatType>(bitfield.Value()), ctx);
    }
};
