...
std::optional<Release> Release::FromJson(const nlohmann::json& json, const std::string& host,
    ...
    const auto release_base =
        std::format("{}/{}/releases", (const char*)Common::g_build_auto_update_website, repo);
    const auto fallback_html = std::format("{}/tag/{}", release_base, rel.tag);
    rel.html_url = json.value("html_url", fallback_html);

    // This is our own "fake" API.
    if (json.contains("base")) {
        const auto base = json.value("base", std::format("https://{}", (const char*)Common::g_build_auto_update_api));
        rel.base_download_url = std::format("{}/{}", base, rel.tag);

        // Assets are easy :)
        rel.assets = json.value("assets", std::vector<std::string>{});
    } else {
        const auto base_download_url = std::format("/{}/releases/download/{}", repo, rel.tag);
...
std::vector<Release> GetReleases() {
    ...
    const auto url = std::format("https://{}", (const char*)Common::g_build_auto_update_stable_api);
    return Release::ListFromJson(body_str, url, (const char*)Common::g_build_auto_update_stable_repo);
}

std::optional<Release> GetLatestRelease() {
    const auto releases_path =  (const char*)Common::g_build_auto_update_api_path;
    const auto url = std::format("https://{}", (const char*)Common::g_build_auto_update_api);
    ...
    const std::string_view body_str = body.value();
    return Release::FromJson(body_str, url, (const char*)Common::g_build_auto_update_repo);
}

std::optional<std::string> GetReleasesBody() {
    const auto releases_path =
        std::format("/{}/{}/releases", (const char*)Common::g_build_auto_update_stable_api_path,
                    (const char*)Common::g_build_auto_update_stable_repo);
    const auto url = std::format("https://{}", (const char*)Common::g_build_auto_update_stable_api);
