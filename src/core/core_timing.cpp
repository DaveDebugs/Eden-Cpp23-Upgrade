...
struct CoreTiming::Event {
    ...
    auto operator<=>(const Event& right) const {
        if (auto cmp = time <=> right.time; cmp != 0) return cmp;
        return fifo_order <=> right.fifo_order;
    }

    bool operator==(const Event& right) const {
        return time == right.time && fifo_order == right.fifo_order;
    }
};
...
void CoreTiming::ScheduleEvent(std::chrono::nanoseconds ns_into_future,
                               const std::shared_ptr<EventType>& event_type, bool absolute_time) {
    {
        ...
        event_queue.push(Event{next_time.count(), event_fifo_id++, event_type, 0, event_type->sequence_number});
    }
...
void CoreTiming::ScheduleLoopingEvent(std::chrono::nanoseconds start_time,
                                      ...
                                      bool absolute_time) {
    {
        ...
        event_queue.push(
            Event{next_time.count(), event_fifo_id++, event_type, resched_time.count(), event_type->sequence_number});
    }
...
std::optional<s64> CoreTiming::Advance() {
    std::lock(advance_lock, basic_lock);
    std::unique_lock<std::mutex> lk_adv(advance_lock, std::adopt_lock);
    std::unique_lock<std::mutex> lk_basic(basic_lock, std::adopt_lock);
    ...
    while (!event_queue.empty() && event_queue.top().time <= global_timer) {
        ...
        if (const auto event_type{evt.type.lock()}) {
            ...
            if (evt.sequence_number != evt_sequence_num) {
                // Event was unscheduled or rescheduled.
                continue;
            }
            ...
            if (evt.reschedule_time == 0) {
                ...
                if (evt_sequence_num != event_type->sequence_number) {
                    // Sequence number was invalidated after external modification.
                    continue;
                }
                ...
                event_queue.push(Event{next_time, event_fifo_id++, evt.type,
                                       next_schedule_time, evt.sequence_number});
            }
        }
