...
PerfStats::~PerfStats() {
    ...
    const auto filepath = path / "benchmark_results.json";

    if (Common::FS::CreateParentDir(filepath)) {
        Common::FS::IOFile file(filepath, Common::FS::FileAccessMode::Write,
                                Common::FS::FileType::TextFile);
        
        double avg_sys_fps = cumulative_stats.samples ? cumulative_stats.total_system_fps / cumulative_stats.samples : 0.0;
        double avg_game_fps = cumulative_stats.samples ? cumulative_stats.total_game_fps / cumulative_stats.samples : 0.0;
        double avg_frametime = cumulative_stats.samples ? cumulative_stats.total_frametime / cumulative_stats.samples : 0.0;
        double avg_speed = cumulative_stats.samples ? cumulative_stats.total_emulation_speed / cumulative_stats.samples : 0.0;

        std::string json = std::format(
            "{{\n"
            "  \"title_id\": \"{:016X}\",\n"
            "  \"average_system_fps\": {:.2f},\n"
            "  \"average_game_fps\": {:.2f},\n"
            "  \"average_frametime_ms\": {:.2f},\n"
            "  \"average_emulation_speed_percent\": {:.2f},\n"
            "  \"total_samples\": {}\n"
            "}}\n",
...
PerfStatsResults PerfStats::GetAndResetStats(microseconds current_system_time_us) {
    ...
    if (current_index > IgnoreFrames) {
        cumulative_stats.total_system_fps += results.system_fps;
        cumulative_stats.total_game_fps += results.average_game_fps;
        cumulative_stats.total_frametime += results.frametime;
        cumulative_stats.total_emulation_speed += results.emulation_speed;
        cumulative_stats.samples++;
    }
