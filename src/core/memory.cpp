...
struct Memory::Impl {
    ...
    u16 Read16(const Common::ProcessAddress addr) {
        if ((addr & 1) == 0) {
            ...
            return std::byteswap(Read<u16_be>(addr));
        }
    }
    ...
    u32 Read32(const Common::ProcessAddress addr) {
        if ((addr & 3) == 0) {
            ...
            return std::byteswap(Read<u32_be>(addr));
        }
    }
    ...
    u64 Read64(const Common::ProcessAddress addr) {
        if ((addr & 7) == 0) {
            ...
            return std::byteswap(Read<u64_be>(addr));
        }
    }
    ...
    inline bool WalkBlock(const Common::ProcessAddress addr, const std::size_t size, F1&& on_unmapped, F2&& on_memory, F3&& on_rasterizer) {
        ...
        while (remaining_size != 0) {
            ...
            // Fast-path: find contiguous chunks of the same type and block
            std::size_t contiguous_pages = 1;
            while (contiguous_pages * YUZU_PAGESIZE - page_offset < remaining_size &&
                   current_page_table->entries[page_index + contiguous_pages].ptr.PointerType() == std::make_pair(pointer + (contiguous_pages << YUZU_PAGEBITS), type)) {
                contiguous_pages++;
            }

            const std::size_t copy_amount = (std::min)(contiguous_pages * YUZU_PAGESIZE - page_offset, remaining_size);
    ...
    void HandleRasterizerDownload(VAddr v_address, size_t size) {
        const auto* p = GetPointerImpl(
            v_address, []() {}, []() {});
        std::call_once(gpu_device_memory_flag, [this]() {
    ...
    std::once_flag gpu_device_memory_flag;
#ifdef __ANDROID__
