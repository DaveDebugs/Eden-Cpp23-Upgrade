...
json GetYuzuVersionData() {
    return {
        {"scm_rev", std::string(static_cast<const char*>(Common::g_scm_rev))},
        {"scm_branch", std::string(static_cast<const char*>(Common::g_scm_branch))},
        {"scm_desc", std::string(static_cast<const char*>(Common::g_scm_desc))},
        {"build_name", std::string(static_cast<const char*>(Common::g_build_name))},
        {"build_date", std::string(static_cast<const char*>(Common::g_build_date))},
        {"build_fullname", std::string(static_cast<const char*>(Common::g_build_fullname))},
        {"build_version", std::string(static_cast<const char*>(Common::g_build_version))},
    };
}
...
json GetProcessorStateData(const std::string& architecture, u64 entry_point, u64 sp, u64 pc,
    ...
    if (backtrace.has_value()) {
        ...
        for (const auto& entry : *backtrace) {
            backtrace_out.push_back(std::format("{:016X}", entry));
        }
...
void Reporter::SaveCrashReport(u64 title_id, Result result, u64 set_flags, u64 entry_point, u64 sp,
    ...
    proc_out["set_flags"] = std::format("{:016X}", set_flags);
    proc_out["afsr0"] = std::format("{:016X}", afsr0);
    proc_out["afsr1"] = std::format("{:016X}", afsr1);
    proc_out["esr"] = std::format("{:016X}", esr);
    proc_out["far"] = std::format("{:016X}", far);
    proc_out["backtrace_size"] = std::format("{:08X}", backtrace_size);
    proc_out["unknown_10"] = std::format("{:08X}", unk10);
