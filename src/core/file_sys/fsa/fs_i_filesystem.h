...
public:
    ...
    std::expected<VirtualFile, Result> OpenFile(const Path& path, OpenMode mode) {
        VirtualFile out_file;
        auto result = this->DoOpenFile(&out_file, path, mode);
        if (result.IsFailure()) {
    ...
    std::expected<VirtualDir, Result> OpenDirectory(const Path& path, OpenDirectoryMode mode) {
        VirtualDir out_dir;
        auto result = this->DoOpenDirectory(&out_dir, path, mode);
        if (result.IsFailure()) {
            return std::unexpected(result);
        }
        return out_dir;
