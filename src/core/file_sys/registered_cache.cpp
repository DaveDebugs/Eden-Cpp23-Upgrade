...
std::string ContentProviderEntry::DebugInfo() const {
    return std::format("title_id={:016X}, content_type={:02X}", title_id, static_cast<u8>(type));
}
...
static std::string GetRelativePathFromNcaID(const std::array<u8, 16>& nca_id, bool second_hex_upper,
    ...
    if (!within_two_digit) {
        const auto format_str = (cnmt_suffix ? "{}.cnmt.nca" : "/{}.nca");
        return std::vformat(format_str, std::make_format_args(nca_str));
    }
    ...
    const auto format_str =
        (cnmt_suffix ? "/000000{:02X}/{}.cnmt.nca" : "/000000{:02X}/{}.nca");

    LOG_DEBUG(Loader, "Decoded {} bytes, nca id {}", hash_len, nca_str);

    return std::vformat(format_str, std::make_format_args(hash[0], nca_str));
}
...
static std::string GetCNMTName(TitleType type, u64 title_id) {
    ...
    return std::format("{}_{:016x}.cnmt", TITLE_TYPE_NAMES[index], title_id);
}
...
bool PlaceholderCache::Create(const NcaID& id, u64 size) const {
    ...
    const auto dirname = std::format("000000{:02X}", hash[0]);
    ...
    const auto file = dir2->CreateFile(std::format("{}.nca", Common::HexToString(id, false)));
...
bool PlaceholderCache::Delete(const NcaID& id) const {
    ...
    const auto res = dir2->DeleteFile(std::format("{}.nca", Common::HexToString(id, false)));
...
VirtualFile RegisteredCache::OpenFileOrDirectoryConcat(const VirtualDir& open_dir,
    ...
    for (std::size_t i = 0; i < 0x100; ++i) {
        auto next = nca_dir->GetFile(std::format("{:02X}", i));
        if (next != nullptr) {
            concat.push_back(std::move(next));
        } else {
            next = nca_dir->GetFile(std::format("{:02x}", i));
            if (next != nullptr) {
                concat.push_back(std::move(next));
            } else {
                break;
...
static std::shared_ptr<NCA> GetNCAFromNSPForID(const NSP& nsp, const NcaID& id) {
    auto file = nsp.GetFile(std::format("{}.nca", Common::HexToString(id, false)));
    if (file == nullptr) {
        return nullptr;
    }
