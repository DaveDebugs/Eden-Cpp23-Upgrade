...
static void GenerateErrorReport(Core::System& system, Result error_code, const FatalInfo& info) {
    ...
    std::string crash_report = std::format(
        ...
        "Result:                          {:#X} ({:0>4d}-{:0>4d})\n"
        "Set flags:                       0x{:0>16X}\n"
        "Program entry point:             0x{:0>16X}\n"
        "\n",
        static_cast<const char*>(Common::g_scm_branch), static_cast<const char*>(Common::g_scm_desc), title_id, error_code.raw,
    ...
    if (info.backtrace_size != 0x0) {
        ...
        for (size_t i = 0; i < info.registers.size(); i++) {
            crash_report +=
                std::format("    X[{:0>2d}]:                       {:0>16x}\n", i, info.registers[i]);
        }
        crash_report += std::format("    SP:                          {:0>16x}\n", info.sp);
        crash_report += std::format("    PC:                          {:0>16x}\n", info.pc);
        crash_report += std::format("    PSTATE:                      {:0>16x}\n", info.pstate);
        crash_report += std::format("    AFSR0:                       {:0>16x}\n", info.afsr0);
        crash_report += std::format("    AFSR1:                       {:0>16x}\n", info.afsr1);
        crash_report += std::format("    ESR:                         {:0>16x}\n", info.esr);
        crash_report += std::format("    FAR:                         {:0>16x}\n", info.far);
        crash_report += "\nBacktrace:\n";
        for (u32 i = 0; i < std::min<u32>(info.backtrace_size, 32); i++) {
            crash_report +=
                std::format("    Backtrace[{:0>2d}]:               {:0>16x}\n", i, info.backtrace[i]);
        }

        crash_report += std::format("Architecture:                    {}\n", info.ArchAsString());
        crash_report += std::format("Unknown 10:                      0x{:0>16x}\n", info.unk10);
    }
