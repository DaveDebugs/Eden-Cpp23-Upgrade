...
Result IFileSystem::OpenFile(OutInterface<IFile> out_interface,
    ...
    auto expected_vfs_file = backend->OpenFile(FileSys::Path(path->str),
                            static_cast<FileSys::OpenMode>(mode));
    if (!expected_vfs_file.has_value()) {
        R_THROW(expected_vfs_file.error());
    }

    *out_interface = std::make_shared<IFile>(system, expected_vfs_file.value());
    R_SUCCEED();
}
...
Result IFileSystem::OpenDirectory(OutInterface<IDirectory> out_interface,
    ...
    auto expected_vfs_dir = backend->OpenDirectory(FileSys::Path(path->str),
                                 static_cast<FileSys::OpenDirectoryMode>(mode));
    if (!expected_vfs_dir.has_value()) {
        R_THROW(expected_vfs_dir.error());
    }

    *out_interface = std::make_shared<IDirectory>(system, expected_vfs_dir.value(),
                                                  static_cast<FileSys::OpenDirectoryMode>(mode));
    R_SUCCEED();
}
