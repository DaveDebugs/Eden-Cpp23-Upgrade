...
static void PrintHelp(const char* argv0) {
    std::print("Usage: {} [options] <filename>\n"
...
static void PrintVersion() {
    std::println("Eden {} {}", static_cast<const char*>(Common::g_scm_branch), static_cast<const char*>(Common::g_scm_desc));
}
...
static void OnStatusMessageReceived(const Network::StatusMessageEntry& msg) {
    std::string message = [&]() {
        ...
        case Network::IdMemberJoin:
            return std::format("{} has joined", msg.nickname);
        case Network::IdMemberLeave:
            return std::format("{} has left", msg.nickname);
        case Network::IdMemberKicked:
            return std::format("{} has been kicked", msg.nickname);
        case Network::IdMemberBanned:
            return std::format("{} has been banned", msg.nickname);
        case Network::IdAddressUnbanned:
            return std::format("{} has been unbanned", msg.nickname);
        default:
            return std::string{};
        }
    }();
...
int main(int argc, char** argv) {
    ...
    static struct option long_options[] = {
        ...
        {"preload-shaders", no_argument, 0, 's'},
        {0, 0, 0, 0},
        // clang-format on
    };

    while (optind < argc) {
        int arg = getopt_long(argc, argv, "g:fhvcip:s:c:u:d:", long_options, &option_index);
        if (arg != -1) {
            switch (char(arg)) {
            case 'd':
            ...
            case 'm': {
                ...
                if (!std::regex_match(str_arg, re)) {
                    std::print("Wrong format for option --multiplayer\n");
                    PrintHelp(argv[0]);
                    return 0;
                }
                ...
                if (!std::regex_match(nickname, nickname_re)) {
                    std::print("Nickname is not valid. Must be 4 to 20 alphanumeric characters.\n");
                    return 0;
                }
                if (address.empty()) {
                    std::print("Address to room must not be empty.\n");
                    return 0;
                }
                break;
            }
    ...
    // Default to synchronous shaders in CLI unless overridden, to ensure benchmark stability
    ...
    if (preload_shaders && Settings::values.use_disk_shader_cache.GetValue()) {
        std::print("Preloading disk shaders... this may take a moment.\n");
        system.Renderer().ReadRasterizer()->LoadDiskResources(
            system.GetApplicationProcessProgramID(), std::stop_token{},
            [](VideoCore::LoadCallbackStage stage, size_t value, size_t total) {
                if (stage == VideoCore::LoadCallbackStage::Build && total > 0 && (value % 50 == 0 || value == total)) {
