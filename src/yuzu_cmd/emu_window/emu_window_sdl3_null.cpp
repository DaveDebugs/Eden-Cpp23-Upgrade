...
EmuWindow_SDL3_Null::EmuWindow_SDL3_Null(InputCommon::InputSubsystem* input_subsystem_,
    ...
    const std::string window_title = std::format("Eden {} | {}-{} (Vulkan)", static_cast<const char*>(Common::g_build_name),
    ...
    SDL_PumpEvents();
    LOG_INFO(Frontend, "Eden Version: {} | {}-{} (Null)", static_cast<const char*>(Common::g_build_name),
             static_cast<const char*>(Common::g_scm_branch), static_cast<const char*>(Common::g_scm_desc));
}
